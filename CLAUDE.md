...
- **Round-dynamic question counts** (`QUESTIONS_BY_ROUND` in `lib/round-types.ts`, shared by the question route and `lib/feedback-config.ts`): technical_screen=5, technical_deep_dive=8, system_design=6, behavioural=7, final=8, hr_screen=5, case_study=5. (This line previously said "screening=5, technical=8, final=10, behavioural=7" — final was wrong (10 vs actual 8), and several round types weren't listed at all.)
...
### Post-Interview Feedback (`claude/prep-signals-pmf-path-wdngr6`) — staging/preview-only, not on main
Direct response to the PMF discussion: nearly every product decision so far (redesigns, tone, conversion
flow) was driven by persona-review agents and self-review, not real users — this form is the first
real-user feedback instrument, deployed as a Vercel preview off this branch only, deliberately not
merged to `main`. See `BACKLOG.md` for the full item list and infra caveats.
| Item | Status |
|---|---|
| Round-and-length-aware feedback question set | ✅ Done — `lib/feedback-config.ts`, deterministic (no LLM), same "deterministic where possible" philosophy as `lib/rubric-researched.ts`. Universal questions on every round; round-specific quality markers (technical vs. system-design vs. conversational-style rounds are graded on different things); one extra question appended for the two 8-question round types (technical_deep_dive/final) about coherence across a long round |
| `post_interview_feedback` table + `GET/POST /api/sessions/[sessionId]/feedback` | ✅ Done — same `assertSessionOwner` auth as the rest of the session API; POST re-validates every answer server-side against the same question-config function the client used, not just trusting client input |
| Rendered in `DebriefReport.tsx` as section "10 — Help Us Improve" | ✅ Done — `tsc`/`eslint`/`vitest` all clean. **Not yet visually verified in a browser** — this sandbox has no `DATABASE_URL`, so the live form (vs. the dev-preview fallback rendering) hasn't been click-tested |
| Vercel Preview ↔ Production DB isolation | ❌ Not done, needs a human with dashboard access — Vercel Preview environments share `DATABASE_URL` with Production unless a separate one is set (ideally a Neon branch DB via the Neon-Vercel integration), so this should be set up **before** real people are pointed at the preview URL, or test feedback will land in production tables |
