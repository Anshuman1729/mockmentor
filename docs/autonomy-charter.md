...
**Product is not flat, for its own reason — approved as proposed, not overridden.** VP Product's hiring spec (`ops/product/hiring-spec.md`) proposed a partial chain rather than defaulting to the flat precedent, and Anshuman approved it exactly as written:
